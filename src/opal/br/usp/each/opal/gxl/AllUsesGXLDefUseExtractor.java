...
public class AllUsesGXLDefUseExtractor {
	...
	private static final Logger logger = LoggerFactory.getLogger(AllUsesGXLDefUseExtractor.class);
	...
	static {
		...
		try {
			...
			logger.error("Error getting template for def-use gxl files names");
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (file.isDirectory()) {
				...
				if (files.length > 0)
					logger.info("Found {} def-use GXL file(s). Computing requirements...", files.length);
				else
					logger.info("None def-use GXL file found.");
				...
				if(!file.getName().matches(defUseName.getRegex())) {
					logger.error("{} does not match with GXL def-use file name pattern", file);
				} else {
	...
	private static void exportDuasForDefUseGXLFile(File defUseFile, File dest) {
		...
		logger.info("Extracting def-use association (dua) from {}", fileName);

		DFGraph graph;
		try {
			graph = new GXLDefUseReader(defUseFile, methodName).getDFGraph("anyClass", 0);
		} catch (Exception e) {
			logger.error("Error reading gxl file {}", defUseFile);
			return;
		}

		Dua[] duas = new DuaDetermination().requirement(graph);
		logger.debug(String.format("%s.%s has %d dua(s)", className, methodName, duas.length));
		if (computeStats)
		...
		try {
			...
			logger.error("Error writing duas for gxl file {}", fileName);
			return;
		}
	}
