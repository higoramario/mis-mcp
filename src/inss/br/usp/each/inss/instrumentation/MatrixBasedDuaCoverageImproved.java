...
public class MatrixBasedDuaCoverageImproved implements Instrumentator {
	...
	public void instrument(Program p, Requirement[] requirement) {
		...
		for (Dua dua : duas) {
			Use use = dua.getUse(); // for each USE insert a probe...
			for (int i = 0; i < uses.length; i++) {
				if (use.equals(uses[i])) {
					...
					if (use.getType() == Type.C_USE) {
						// c-use
						e.addFirst(new UseProbe(i, dua.getVariable(), vars, m));
					} else {
						// p-use
						e.addFirst(new PUseProbe(i, dua.getVariable(), vars, m, 
