...
abstract class AllUsesInformation implements Information {
	...
	public static final Logger logger = LoggerFactory.getLogger(AllUsesInformation.class);
		...
		for (int posMethod = 0; posMethod < methods.length; posMethod++) {
			...
			for (Object obj : allUses.getRequirements()) {
				...
				if (defUse.getUseTo() != null) { // p-use
					...
					logger.error("Fail to generate dua for:{}.{}", clazz.getClassName(), method.getName());

					if (defUse.getUseTo() != null) { // p-use
						logger.error("Puse:{}", defUse);						
					} else { // c-use
						logger.error("Cuse:{}", defUse);
					}					
				}
